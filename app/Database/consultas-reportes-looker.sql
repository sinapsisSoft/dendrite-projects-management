...
BEGIN
  SELECT PR.ProjReq_id, PR.User_id, U.User_name AS Manager_name, C.Client_name, PR.ProjReq_name, PR.Brand_id, B.Brand_name, PR.ProjReq_observation, PR.Stat_id, S.Stat_name, PR.Project_id, P.Project_code, P.User_id, U1.User_name, PR.created_at, PRP.Prod_id, PD.Prod_name FROM project_request PR
  ...
  LEFT JOIN user U1 ON P.User_id = U1.User_id
  LEFT JOIN project_request_product PRP ON PRP.ProjReq_id = PR.ProjReq_id
...
-- Bar chart of each collaborator indicating the average completeness of their activities
DELIMITER $$
DROP PROCEDURE IF EXISTS `sp_select_traffic_info_chart1`$$
CREATE PROCEDURE `sp_select_traffic_info_chart1` (IN initDate DATE, IN finDate DATE, IN userId INT)   
BEGIN
  SELECT U.User_name, ROUND(AVG(SubAct_percentage),2) AS User_average FROM subactivities SA
  INNER JOIN activities AC ON SA.Activi_id = AC.Activi_id
  INNER JOIN project_product PP ON PP.Project_product_id = AC.Project_product_id
...
-- Polar area chart on the percentage of progress of the projects
DELIMITER $$
DROP PROCEDURE IF EXISTS `sp_select_traffic_info_chart2`$$
CREATE PROCEDURE `sp_select_traffic_info_chart2` (IN initDate DATE, IN finDate DATE, IN userId INT)   
BEGIN
SELECT P.Project_name, P.Project_percentage FROM subactivities SA
  INNER JOIN activities AC ON SA.Activi_id = AC.Activi_id
  INNER JOIN project_product PP ON PP.Project_product_id = AC.Project_product_id
...
-- Grafico de lienas de la cantidad de proyectos por cliente y mes
DELIMITER $$
DROP PROCEDURE IF EXISTS `sp_select_traffic_info_chart3`$$
CREATE PROCEDURE `sp_select_traffic_info_chart3` (IN initDate DATE, IN finDate DATE, IN userId INT)   
BEGIN
  SELECT DATE_FORMAT(P.Project_startDate, '%Y-%m') AS Project_date, COUNT(P.Project_id) AS Project_count, P.Client_id, C.Client_name FROM project P
  INNER JOIN client C ON P.Client_id = C.Client_id
  WHERE (P.Project_startDate BETWEEN initDate AND finDate) AND P.User_id = userId
...
-- Directive Role reports
-- Table information
DELIMITER $$
DROP PROCEDURE IF EXISTS `sp_select_directive_info_table`$$
CREATE PROCEDURE `sp_select_directive_info_table` (IN initDate DATE, IN finDate DATE)   
BEGIN
  SELECT PR.ProjReq_id, PR.User_id, U.User_name AS Manager_name, C.Client_name, PR.ProjReq_name, PR.Brand_id, B.Brand_name, PR.ProjReq_observation, PR.Stat_id, S.Stat_name, PR.Project_id, P.Project_code, P.User_id, U1.User_name, PR.created_at, PRP.Prod_id, PD.Prod_name FROM project_request PR
  INNER JOIN user U ON PR.User_id = U.User_id
  INNER JOIN user_manager UM ON U.User_id = UM.User_id
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `sp_select_directive_info_chart1`$$
CREATE PROCEDURE `sp_select_directive_info_chart1` (IN initDate DATE, IN finDate DATE)   
BEGIN
  SELECT CONCAT(Project_code, " ", SUBSTRING(Brand_name, 1, 10)) AS Project_name, 
  CASE 
    WHEN Project_activitiEndDate > 0 THEN ROUND((DATEDIFF (Project_estimatedEndDate, Project_activitiEndDate) * 100) / DATEDIFF (Project_estimatedEndDate, Project_startDate),2) + 100 
...
-- Lineas graphic of prject numbers by client and month
DELIMITER $$
DROP PROCEDURE IF EXISTS `sp_select_directive_info_chart3`$$
CREATE PROCEDURE `sp_select_directive_info_chart3` (IN initDate DATE, IN finDate DATE)   
BEGIN
  SELECT DATE_FORMAT(PR.created_at, '%Y-%m') AS Project_date, COUNT(PR.ProjReq_id ) AS Project_count, PR.User_id, M.Manager_name FROM project_request PR
  INNER JOIN user U ON PR.User_id = U.User_id
  INNER JOIN user_manager UM ON U.User_id = UM.User_id
